...
The data are in the `Duncan` data set in the **car** package, and so we load the package to access and examine the data:
```{r}
library("car")      
...
summary(Duncan) 
```
Notice that there's a fourth variable in the data set, type of occupation (`type`, a categorical variable or *factor* in R), which I added, and which has three categories (called *levels* in R): blue-color (`"bc"`) , while-collar (`"wc"`) and professional and managerial (`"prof"`). The names of the occupations define the row-names of the data set and the variables define the columns.
...
An examination of the studentized residuals from the regression reveals no obvious problems:
```{r}
